...
| OGS-002 cache boundary excludes remote/AI content | service-worker unit + network inspection | `offline-shell.test.js` passed: explicit core list, GET-only path, same-origin restriction, Worker/model exclusion, network-first navigation, and active-cache offline fallback. Native Chrome’s `maestros-secret-shell-v1` inventory is a historical desktop checkpoint. | automated pass; network-inspection manual pending |
| OGS-003 update handoff protects active chronicles | service-worker simulation + lifecycle test + browser lifecycle test | `offline-shell.test.js` executes the worker lifecycle and proves that accepting an update deletes no old cache, claims no other clients, and posts `UPDATE_READY` only to the requester. `pwa-lifecycle.test.js` proves the requester reloads only after that message. Full browser suite passed 45/45. | automated pass; device update manual pending |
