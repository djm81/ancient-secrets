...
3. **local** — new `js/local-inference.js`.

The local backend detects, in order: Chrome built-in Prompt API (`LanguageModel` availability; zero additional download in supporting browsers), WebLLM (WebGPU required), transformers.js (WASM fallback). Detection is lazy — it runs only when the player opens the backend setting or has previously opted in. Every detection failure resolves to the authored path without error surfacing beyond the settings status line.

## 2. Opt-in, download, and eviction lifecycle

- Selecting the local backend shows a disclosure: what downloads (engine + weights), from where (the engine's own distribution), approximate size, and that everything stays on the device.
- Download begins only on explicit confirmation; progress is announced; cancellation reverts to the previous backend.
- Assets live in browser-managed caches (Cache Storage / IndexedDB as used by the engine); an "evict local model" control clears them and reverts the preference.
- The site payload never includes model assets; the repository never commits them.
...
- GR-002: every local reply passes the unchanged `validateGuidance` plus the hallucination/spoiler validator from `ai-guardrail-evals`; rejection serves the authored hint.
- GR-003: authored hints remain default and complete.
- GR-004: missing WebGPU, failed downloads, engine crashes, and low-memory aborts fail closed.
...
- Engines load in a worker thread where the engine supports it, keeping the main thread interactive.
...
Delete `js/local-inference.js`, the backend registration, and the settings entry. Preference fields in saves degrade safely (unknown backend → authored). No other surface changes.
