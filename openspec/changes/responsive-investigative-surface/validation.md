...
Status: **in progress** — automated, iPhone Safari physical interaction, and limited native desktop Safari/Chrome interaction evidence is recorded. Android Chrome physical validation and the full Firefox run are user-approved skips; native desktop Edge/Safari full-loop evidence and VoiceOver announcement evidence remain outstanding.
...
| RI-006 player-controlled Casebook | browser + native Chrome/Safari | `RI-006: player-controlled Casebook starts closed and preserves the active chronicle` passed on 2026-07-29; full browser suite passed 45/45 and accessibility suite 3/3. Local native Chrome and Safari both exposed collapsed **SHOW CASEBOOK**, expanded to **HIDE CASEBOOK**, and revealed the Casebook action groups. | automated and native desktop smoke pass |

## Native desktop browser interaction evidence — 2026-07-29

- In a fresh native **Safari** tab, the deployed game loaded and rendered `Casebook: desktop landscape`. Starting the chronicle and selecting **Hand Mirror** changed the objective from “Take the hand mirror from the workshop wall” to “Use the hand mirror on the backwards-written note.” Safari reused existing site storage, so this was a smoke path rather than a clean-profile onboarding or a full-loop record.
- In a fresh native **Google Chrome** tab, the deployed game completed **Begin the Adventure** → **Begin Exploring** → desktop-landscape Casebook. Selecting **Hand Mirror** advanced the same objective and exposed the satchel control.
- These checks were performed through the native browser UI, not Playwright. They do not supply the browser-version, viewport, accessibility-announcement, or full-loop evidence required to close C1/B2; Chrome is supplementary because C1 still needs native Edge and Safari coverage.
