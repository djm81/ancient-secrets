...
test('EC-002 and EC-003: older chronicle saves gain a fresh expedition and malformed expedition data preserves Florence', () => {
  const state = createInitialState();
  const run = createRun(() => 0.25);
  const v1 = { version: 1, savedAt: '2026-07-20T12:00:00+02:00', state: { ...state, dialogue: undefined, notes: undefined }, run };
  const v2 = { version: 2, savedAt: '2026-07-20T12:00:00+02:00', state, run };
  const migratedV1 = parseSave(v1);
  const migratedV2 = parseSave(v2);
  assert.equal(migratedV1.expedition.eras.babylon.status, 'available');
  assert.equal(migratedV2.expedition.eras.babylon.status, 'available');
